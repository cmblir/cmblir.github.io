...
<script src="https://gist.github.com/godhin/fe2f93c5112210015a3f3b3e7ed833df.js"></script>
...
1. 오랜만에 카카오문제를 본 것 같다. 해당 문제에서 원하는 것은 특정 문자에 순위를 매겨서 값을 도출하는 것이다.
2. 딕셔너리를 생성한 다음 각각의 값을 0으로 지정해준다.
    - 이는 collection의 `defaultdict` 함수를 이용해도 된다.
3. 반복문을 통해 설문지와 선택한 것을 동시에 받는다.
    - `zip`을 사용해서 두 값을 동시에 해결하면 된다.
4. 그래서 만약 선택지가 4보다 작다면, 4에서 해당 선택값을 빼준다.
    - 아니라면 선택값에서 4를 뺴준다.
    - 이유는 위에서부터 3, 2, 1, 0, 1, 2, 3 순으로 이어지기 때문이며, 이 외의 4나 7이상의 숫자는 무의미한 투표이므로 배제한다.
5. 다시 반복문을 통해서 우리가 비교해야하는 2가지 값(각각의 설문지 선택)을 비교해서 최종 결과에 반영한다. 이때 우리가 비교해야하는 값들은 적으므로 그냥 리스트 만들때 순서대로 해주면 알아서 문자순으로 들어간다.
    - 만약 문제가 조금 더 복잡했다면 `sorted()`함수로 해결했을 것 같다.