...
excerpt: "프로그래머스 신규 아이디 추천 문제 풀이"
...
<script src="https://gist.github.com/cmblir/67206baf99a0a96d63b94489fc31940b.js"></script>

- 주석을 참고하면서 이해를 돕습니다.
---

## 4. 알고리즘 및 해설
1. 반복문을 통해 해당 값을 조건에 맞게끔 바꿔준다.
    - 1단계 소문자 치환
    - 2단계 문자 제거
    - 3단계 연속된 마침표 치환
    - 4단계 첫과 끝 마침표 제거
    - 5단계 이후 빈 문자열일 경우 a로 변경
    - 6단계 길이를 15로 조절해주기
    - 7단계 길이가 2이하인 경우 해당 id는 리스트형태로 바뀌고 길이가 3이 될때까지 값을 반복한다.
2. 해당 값을 출력해준다.
...
def solution(new_id):
    ...
    return st
```
[정답 출처 : 프로그래머스 다른사람의 풀이]

- 문제해결을 정규표현식과 컴프리핸션으로 아주 깔끔하게 표현했다는 점이 인상깊었다.