...
<script src="https://gist.github.com/cmblir/8c407ff96c96bf1477376d1c1ef4d98b.js"></script>
...
1. 문제의 요점은 각 문자마다 조건에 맞게 값을 도출하여 최종 결과를 만드는 것이다.
2. 딕셔너리 형태로 싱글, 더블, 트리플에 제곱수를 만들었다.
    - 해당 방식 말고도 하나하나 리스트를 만들거나, 그냥 구현해도 무관하다.
3. 이후 10 같은 경우 1, 0 으로 문자가 나뉘어지기때문에 A라는 임의의 문자로 변경하여 추후 10으로 인식하게끔 하였다.
4. 반복문이 도는 동안 isdigit() 함수로 수인지 확인하거나 아까 만든 A라는 수가 있을 경우 스택에 값을 넣는다.
5. 숫자가 아닌 경우
    - 조건에 맞게 계산해준다.
        - 단, 마지막 *의 경우 스택에 수가 남아있을 경우 *2를 더해준다.