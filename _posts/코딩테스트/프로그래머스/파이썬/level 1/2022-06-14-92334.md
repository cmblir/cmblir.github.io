...
<script src="https://gist.github.com/cmblir/ffb94cdaafd2d0903c74f9af85b56900.js"></script>
...
- 해당 문제의 경우 특정 조건이 있다고 판단되지 않아 브루트 포스로 해결하였다.

2. 문제의 요점

- 해당 문제에서 해결해야할 점은 **신고한 사람과 신고당한 사람**의 횟수를 세는 것이다. 이것이 K라는 특정 값을 넘을 경우에만 결과가 나오게끔 유도하는 것이다.

1. 문제 풀기에 앞서 중복(악의적으로 무지성 신고를 하는 경우)를 제외하고 시작한다.
2. 위의 의사코드 작성처럼 신고 명단과 신고당한 사람을 셀 수 있는 dictionary를 만들어준다.
3. 2개의 딕셔너리에서 명단에는 신고자와 신고당한 사람을 세주고, 신고당한 횟수 명단에는 숫자를 세준다.
4. 마지막으로 특정값을 충족하였을 때 결과값에 출력을 해준다.
