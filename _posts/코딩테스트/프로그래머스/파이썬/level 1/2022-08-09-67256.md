...
<script src="https://gist.github.com/cmblir/26cb9b6486c1ce239d480f6873f9a8db.js"></script>
...
1. 해당 문제의 경우 단순 브루트포스 문제로 천천히 풀어야한다.
    - 흔히 브루트포스 문제는 길을 잃으면 끝이라는 말이 있다. 그래서 문제를 시작하기 전에 최대한 플로우를 그리고 하는 것이 좋다. 아니면 의사코드라도 대충 작성하고 하는 것도 하나의 방법이다.
2. 문제 시작전 미리 키패드의 좌표를 만들었다.
3. 시작 지점을 만든 후 해당 값을 업데이트하는 방식으로 진행하였다. 이때 각각의 조건문을 만들어준다.
    - 왼손에서 시작한 경우, 오른손에서 시작한 경우
    - 중간에서 시작한 경우
        - 중간에서 시작해서 이동했을 때 왼손에 더 가까운 경우
        - 중간에서 시작해서 이동했을 때 오른손에 더 가까운 경우
        - 두 손 거리가 같은 경우
            - 왼손잡이인 경우
            - 오른손 잡이인 경우
4. 이후 해당 값들을 반복문을 통해 업데이트해준 뒤 결과값에 입력해준다.
5. 최종 결과값을 출력한다.
