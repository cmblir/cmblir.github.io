...
<script src="https://gist.github.com/godhin/c81d0767ca0f3c43f210e548bde22a64.js"></script>
...
1. 문제에서 이해가 필요한 것은 바로 H index를 이해하는 것이다.
2. 위 문제의 말을 쉽게 풀면 '정렬된 리스트내의 위치값가 위치에 존재하는 값보다 크다면 해당 위치값을 반환해라' 라고 해석할 수 있다.
3. 그래서 정렬된 리스트를 `enumerate()` 함수를 이용해서 위치값과 해당 위치에 존재하는 값을 동시에 사용하게 하고, 조건을 부여하여 맞는다면 해당 조건에 알맞은 값을 반환하게 한다.